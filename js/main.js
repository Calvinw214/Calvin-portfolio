...
(function () {
  ...
  const COMMANDS = [
    ...
    { id: 'nav-skills',  label: 'Go to Skills',         hint: 'Section', action: () => scrollToId('skills') },
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      const endpoint = DATA.contact?.endpoint || null;
      ...
      try {
        ...
        if (res.ok && payload && payload.ok) {
          ...
          const reason = (payload && payload.error) ? String(payload.error) : `Server responded with ${res.status}.`;
        ...
        if (statusEl) {
          ...
          statusEl.innerHTML =
            ...
            `<a class="form-status-link" href="${escapedFallback}">Open your email app instead</a>.`;
        }
      } finally {
